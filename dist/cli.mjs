#!/usr/bin/env node
import { t as KhaanClient } from "./client-H2qqfaK9.mjs";
...
async function rememberDevice(values) {
  const username = values.username ?? (await prompt("Username: "));
  const password = values.password ?? (await prompt("Password: "));
  let deviceId = values["device-id"];
  if (!deviceId)
  ...
  const accountNumber = values["account-number"] ?? "";
